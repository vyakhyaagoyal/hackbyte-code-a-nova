...
// src/svelte/SpacetimeDBProvider.ts
...
// src/svelte/connection_state.ts
var SPACETIMEDB_CONTEXT_KEY = Symbol("spacetimedb");

// src/svelte/SpacetimeDBProvider.ts
var connRef = null;
var cleanupTimeoutId = null;
function createSpacetimeDBProvider(connectionBuilder) {
  ...
  svelte.setContext(SPACETIMEDB_CONTEXT_KEY, store$1);
  svelte.onDestroy(() => {
    connRef?.removeOnConnect(onConnect);
    connRef?.removeOnDisconnect(onDisconnect);
    connRef?.removeOnConnectError(onConnectError);
...
function useSpacetimeDB() {
  const context = svelte.getContext(
    SPACETIMEDB_CONTEXT_KEY
  );
  if (!context) {
    throw new Error(
      "useSpacetimeDB must be used within a component that called createSpacetimeDBProvider. Did you forget to call `createSpacetimeDBProvider` in a parent component?"
    );
  }
  return context;
}
...
function useTable(query, callbacks) {
  ...
  try {
    ...
    throw new Error(
      "Could not find SpacetimeDB client! Did you forget to call `createSpacetimeDBProvider`? `useTable` must be used in a Svelte component tree under a component that called `createSpacetimeDBProvider`."
    );
  }
  const rows = store.writable([]);
  const isReady = store.writable(false);
  let latestTransactionEvent = null;
  let unsubscribeFromTable = null;
  let subscriptionHandle = null;
  const computeFilteredRows = () => {
    const state = store.get(connectionStore);
...
exports.createSpacetimeDBProvider = createSpacetimeDBProvider;
