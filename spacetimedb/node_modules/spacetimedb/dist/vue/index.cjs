...
// src/vue/SpacetimeDBProvider.ts
...
// src/vue/connection_state.ts
var SPACETIMEDB_INJECTION_KEY = Symbol(
  "spacetimedb"
);
...
function setupConnection(connectionBuilder) {
  ...
  vue.provide(SPACETIMEDB_INJECTION_KEY, state);
  let onConnectCallback = null;
  let onDisconnectCallback = null;
  let onConnectErrorCallback = null;
  vue.onMounted(() => {
    if (cleanupTimeoutId) {
    ...
    onConnectCallback = (conn2) => {
      state.isActive = conn2.isActive;
      state.identity = conn2.identity;
      state.token = conn2.token;
      state.connectionId = conn2.connectionId;
      state.connectionError = void 0;
    };
    onDisconnectCallback = (ctx) => {
      state.isActive = ctx.isActive;
    };
    onConnectErrorCallback = (ctx, err) => {
      state.isActive = ctx.isActive;
  ...
  const cleanup = () => {
    if (connRef) {
      ...
      if (onDisconnectCallback) {
        connRef.removeOnDisconnect?.(onDisconnectCallback);
      }
      if (onConnectErrorCallback) {
        connRef.removeOnConnectError?.(onConnectErrorCallback);
      }
...
var SpacetimeDBProvider = vue.defineComponent({
...
function useSpacetimeDBProvider(connectionBuilder) {
  const { state } = setupConnection(connectionBuilder);
  return state;
}
function useSpacetimeDB() {
  const context = vue.inject(SPACETIMEDB_INJECTION_KEY);
  if (!context) {
    throw new Error(
...
function useTable(query, callbacks) {
  ...
  try {
    ...
    throw new Error(
      "Could not find SpacetimeDB client! Did you forget to add a `SpacetimeDBProvider`? `useTable` must be used in a Vue component tree under a `SpacetimeDBProvider` component."
    );
  }
  const rows = vue.shallowRef([]);
  const isReady = vue.ref(false);
  let latestTransactionEvent = null;
  let unsubscribeFromTable = null;
  ...
  const computeFilteredRows = () => {
    ...
    if (!table) return [];
    const allRows = Array.from(table.iter());
    if (whereExpr) {
  ...
  const setupTableListeners = () => {
    ...
    const onUpdate = (eventCtx, oldRow, newRow) => {
  ...
  const setupSubscription = () => {
    const connection = conn.getConnection();
    if (!connection) return;
    subscriptionHandle = connection.subscriptionBuilder().onApplied(() => {
      isReady.value = true;
      rows.value = computeFilteredRows();
    }).subscribe(querySql);
  };
  ...
  return [vue.readonly(rows), vue.readonly(isReady)];
}
function useReducer(reducerDef) {
  const conn = useSpacetimeDB();
  const reducerName = reducerDef.accessorName;
  const queueRef = vue.shallowRef([]);
  const stopWatch = vue.watch(
    () => conn.isActive,
    () => {
      const connection = conn.getConnection();
      if (!connection) return;
      const fn = connection.reducers[reducerName];
...
exports.useSpacetimeDBProvider = useSpacetimeDBProvider;
